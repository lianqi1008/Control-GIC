...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        # z to block_in
        self.conv_in_coarse = torch.nn.Conv2d(z_channels,
                                       block_in,
                                       kernel_size=3,
                                       stride=1,
                                       padding=1)
        ...
        self.mid_coarse.block_1 = ResnetBlock(in_channels=block_in,
                                       out_channels=block_in,
                                       temb_channels=self.temb_ch,
                                       dropout=dropout,
                                       zq_ch=zq_ch,
                                       add_conv=add_conv)
        self.mid_coarse.attn_1 = AttnBlock(block_in, zq_ch, add_conv=add_conv)
        self.mid_coarse.block_2 = ResnetBlock(in_channels=block_in,
                                       out_channels=block_in,
                                       temb_channels=self.temb_ch,
                                       dropout=dropout,
                                       zq_ch=zq_ch,
        ...
        # z to block_in
        self.conv_in_fine = torch.nn.Conv2d(z_channels,
                                       block_in,
                                       kernel_size=3,
                                       stride=1,
                                       padding=1)
        ...
        self.mid_fine.block_1 = ResnetBlock(in_channels=block_in,
                                       out_channels=block_in,
                                       temb_channels=self.temb_ch,
                                       dropout=dropout,
                                       zq_ch=zq_ch,
                                       add_conv=add_conv)
        self.mid_fine.attn_1 = AttnBlock(block_in, zq_ch, add_conv=add_conv)
        self.mid_fine.block_2 = ResnetBlock(in_channels=block_in,
                                       out_channels=block_in,
                                       temb_channels=self.temb_ch,
                                       dropout=dropout,
                                       zq_ch=zq_ch,
        ...
        self.avgpool_layer1 = torch.nn.AvgPool2d(4, 4, 0)
        self.avgpool_layer2 = torch.nn.AvgPool2d(2, 2, 0)
    ...
    def forward(self, z, zq, mask):
        ...
        # z to block_in
        h_coarse = self.conv_in_coarse(z)
        h_medium = self.conv_in(z)
        h_fine = self.conv_in_fine(z)

        # middle
        h_coarse_reconv = self.mid_coarse.block_1(h_coarse, temb, zq)
        h_coarse_reconv = self.mid_coarse.attn_1(h_coarse_reconv, zq)
        h_coarse_reconv = self.mid_coarse.block_2(h_coarse_reconv, temb, zq)

        # middle
        h_medium_reconv = self.mid.block_1(h_medium, temb, zq)
        h_medium_reconv = self.mid.attn_1(h_medium_reconv, zq)
        h_medium_reconv = self.mid.block_2(h_medium_reconv, temb, zq)

        # middle
        h_fine = self.mid_fine.block_1(h_fine, temb, zq)
        h_fine = self.mid_fine.attn_1(h_fine, zq)
        h_fine = self.mid_fine.block_2(h_fine, temb, zq)
        
        h_coarse_reconv = self.avgpool_layer1(h_coarse_reconv)
